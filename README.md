...
Poppler를 번들하지 않으려면 `electron-app/electron-builder.yml`의 `extraResources`에서 `poppler-windows` 블록을 제거하거나 주석 처리하면 됩니다. **PDF 텍스트 추출은 pdftotext만 사용**하므로, Poppler를 포함하지 않는 빌드에서는 최종 사용자 PC에 **pdftotext(poppler)가 PATH에 있어야** PDF 파싱이 동작합니다.

#### pdftotext 없을 때

PDF 파싱은 **pdftotext(poppler)만** 사용합니다(레거시 pdfminer.six·PyMuPDF 기반 경로는 제거됨). Windows는 빌드 시 poppler-windows 번들을 권장합니다. 개발용 `requirements-pdf.txt`는 별도 스크립트·실험용일 수 있으며, 앱 런타임 PDF 파이프라인과는 다를 수 있습니다.
...
- **점수 가중치 설정**: 경력 / 필수 요구사항 / 우대 사항 / 자격증 네 항목의 **가중치 비율** 조정 (종합 점수 계산에 반영)
...
- **엑셀로 내려받기**: 결과 화면에서 후보를 체크한 뒤 **엑셀로 내려받기** 실행. 컬럼 예시 — 이름, 나이, 주소, 최종학력(학교·학과), 최근 경력(회사·부서·연봉), AI등급(상/중/하), **종합점수**(가중치 반영 숫자), 경력적합도·필수·우대·자격 만족도(◎/○/X 등). 필수 요구사항이 **X**이면 종합점수는 **0**
...
- **종합 점수**: AI 등급(A/B/C → 100/80/60)과 필수·우대·자격 만족도 점수에 **채용 설정의 가중치**를 곱해 합산한 **0~100 근처의 숫자**로 표시. 필수 요구사항이 **X**(불만족)이면 다른 항목 점수와 관계없이 **0**

## 점수 체계 (앱 vs Core 모듈)

앱 결과 화면의 **종합 점수**는 `result.totalScore` 같은 저장 필드가 아니라, **AI가 반환한 `evaluations`·등급과 채용 설정 가중치로 매번 계산**한 값입니다(엑셀보내기도 동일 규칙).

아래 **Core 모듈**의 자격증·경력·학력 점수는 **라이브러리·배치 스크립트**용 알고리즘이며, Electron 앱의 메인 평가 화면에서는 사용하지 않습니다.
...
│   └── scoring.ts          # 점수 계산 알고리즘 (앱 UI 평가와 별개)
